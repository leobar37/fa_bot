...
export class Client {
  ...
  public sendImageMessage(id: string, imageUrlOrId: string, cb?: Function) {
    return this.sendUrlOrIdBasedMessage(
      id,
  ...
  public sendButtonsMessage(
    ...
    const payload = {
      ...
      payload: { text, buttons, template_type: 'button' }
    };
  ...
  public sendTemplateMessage(
    id: string,
    templatePayload: IMessageTemplate,
    cb?: Function
  ) {
    const payload = {
      type: ATTACHMENT_TYPE.TEMPLATE,
   ...
   * @return {Promise<any>}
   */
  public sendQuickReplyMessage(
    id: string,
    textOrAttachment: string | AttachmentPayload,
